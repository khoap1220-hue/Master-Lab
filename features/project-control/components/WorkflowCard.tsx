...
import { Workflow, AgentRole } from '../../../types';
...
const WorkflowCard: React.FC<WorkflowCardProps> = ({ workflow }) => {
  ...
  return (
    <div className="rounded-[1.5rem] border border-zinc-800 bg-[#0f172a]/80 backdrop-blur-xl overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500 group">
      
      {/* HEADER */}
      <div className="p-5 border-b border-zinc-800/50 flex items-start justify-between bg-gradient-to-r from-zinc-900 via-zinc-900 to-transparent">
        <div>
          <div className="flex items-center gap-2 mb-1">
             <div className={`w-2 h-2 rounded-full ${workflow.status === 'executing' ? 'bg-green-500 animate-pulse' : workflow.status === 'completed' ? 'bg-blue-500' : 'bg-zinc-500'}`}></div>
             <h4 className="text-xs font-black text-white uppercase tracking-widest">{workflow.name}</h4>
          </div>
          <p className="text-[10px] font-mono text-zinc-400">{getStatusLabel(workflow.status)}</p>
        </div>
        
        <div className="text-right">
           <span className="text-2xl font-black text-white block leading-none">{workflow.progress}%</span>
           <span className="text-[8px] text-zinc-500 uppercase font-bold tracking-wider">Completion</span>
        </div>
      </div>
      ...
      <div className="p-5 space-y-6">
          ...
          <div className="relative overflow-hidden rounded-xl border border-orange-500/30 bg-orange-500/5 p-4 animate-pulse-slow">
             ...
             <div className="relative z-10">
                <div className="flex items-center gap-2 mb-2">
                   ...
                   <span className={`text-[10px] font-black uppercase ${AGENT_COLORS[activeTask.assignedAgent] || 'text-zinc-400'}`}>{activeTask.assignedAgent}</span>
                </div>
        ...
        {(workflow.status === 'executing' || workflow.status === 'planning') && showTerminal && (
           <div className="rounded-xl bg-black/50 border border-zinc-800 p-3 font-mono">
              <div className="flex items-center justify-between mb-2 pb-2 border-b border-zinc-800/50">
                 <span className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                    <span className="w-1 h-1 rounded-full bg-green-500 animate-ping"></span>
                    Live Neural Stream
                 </span>
                 <Button variant="ghost" onClick={() => setShowTerminal(false)} className="text-[8px] text-zinc-600 hover:text-zinc-400 h-auto p-0">Hide</Button>
              </div>
        ...
        <div className="space-y-0 relative">
           {/* Connecting Line */}
           <div className="absolute left-[11px] top-2 bottom-2 w-px bg-zinc-800 z-0"></div>
           ...
           {(workflow.tasks || []).slice().reverse().slice(0, 5).map((task, idx) => (
             <div key={task.id} className="relative z-10 flex gap-3 group">
                <div className={`mt-1.5 w-6 h-6 rounded-full flex items-center justify-center border-2 transition-all ${
                   task.status === 'completed' ? 'bg-zinc-900 border-green-500 text-green-500' :
                   ...
                   {task.status === 'pending' && <div className="w-2 h-2 bg-zinc-700 rounded-full"></div>}
                </div>
                ...
                <div className={`flex-1 pb-4 transition-all ${task.status === 'pending' ? 'opacity-40 grayscale' : 'opacity-100'}`}>
                   <div className="flex items-center justify-between">
                      <span className={`text-[9px] font-black uppercase tracking-wider mb-0.5 px-2 py-0.5 rounded border ${
                         ...
                         AGENT_COLORS[task.assignedAgent] || 'bg-zinc-800 text-zinc-400 border-zinc-700'
                      }`}>
                         {task.assignedAgent}
                      </span>
                      <span className="text-[8px] text-zinc-600 font-mono">{task.status}</span>
                   </div>
                   <p className={`text-[10px] font-bold mt-1 ${task.status === 'completed' ? 'text-zinc-400 line-through' : 'text-zinc-200'}`}>
                      {task.name}
                   </p>
                </div>
             </div>
           ...
           {(workflow.tasks || []).length > 5 && (
              <div className="text-center text-[9px] text-zinc-600 italic py-2">
        ...
        {/* RESULTS GRID */}
        {hasResults && (
          <div className="pt-4 border-t border-zinc-800 animate-in fade-in slide-in-from-bottom-2">
             <div className="flex items-center justify-between mb-3">
                <span className="text-[9px] font-black uppercase text-zinc-400 tracking-widest">Deliverables</span>
                <Button 
                   variant="ghost"
                   onClick={handleDownloadAll}
                   className="text-[9px] font-bold text-blue-400 hover:text-white flex items-center gap-1 transition-colors h-auto p-0"
                >
             ...
             <div className="grid grid-cols-4 gap-2">
                {workflow.resultImages!.map((img, idx) => (
                  <div key={idx} className="relative group aspect-square rounded-lg overflow-hidden border border-zinc-700 cursor-pointer" onClick={() => handleDownload(img, idx)}>
