...
import { BRANDING_PACKS } from '../config';
...
interface BrandingSectionProps {
  ...
  config: any;
  goal: string; // Added prop for context
}
...
const BrandingSection: React.FC<BrandingSectionProps> = ({
  ...
  const handleMagicSuggest = async () => {
    ...
    try {
      const suggestions = await suggestDeliverables(goal, budgetTier);
      if (suggestions && suggestions.length > 0) {
        setDeliverablesList(suggestions.join(', '));
      }
    } catch (error) {
    ...
    <div>
      <div className="flex justify-between items-center mb-2">
         <label className="text-[9px] font-bold text-slate-500 uppercase block">
            {config.deliverablesLabel || 'Danh sách hạng mục'}
         </label>
         {deliverablesList && (
            <span className="text-[8px] text-blue-400 font-bold">{(deliverablesList || '').split(',').length} hạng mục đã chọn</span>
         )}
      </div>
      ...
      >
        {isSuggesting ? (
          <>
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">AI Đang phân tích POSM & Chi phí...</span>
          </>
        ) : (
          <>
            <span className="text-lg">✨</span>
            <span className="text-[9px] font-black uppercase tracking-widest">
              {goal.trim() ? "Gợi ý thông minh (Theo mức đầu tư)" : "Nhập mô tả để AI gợi ý"}
            </span>
          </>
        )}
      </button>
      ...
      <textarea 
         ...
         placeholder={config.deliverablesPlaceholder}
