    ...
    <>
      <div>
         <div className="flex items-center gap-2 mb-2 group relative w-fit">
            <label className="text-[9px] font-bold text-zinc-500 uppercase flex items-center gap-2 cursor-help">
            ...
            {/* TOOLTIP */}
            <div className="absolute bottom-full left-0 mb-2 w-48 p-2 bg-zinc-800 text-[9px] text-zinc-300 rounded-lg border border-zinc-700 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
               Thêm các phong cách nghệ thuật hoặc kỹ thuật cụ thể vào thiết kế (VD: Ánh sáng neon, Chất liệu gỗ).
            </div>
            <span className="text-[8px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400">Tùy chọn</span>
         </div>
         ...
         <div className="flex flex-wrap gap-2">
            {injectors.map(tag => (
               <Button 
                 ...
                 className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wide border transition-all h-auto ${
         ...
         <div>
           <div className="flex items-center justify-between mb-2">
              <div className="group relative flex items-center gap-2">
                  <label className="text-[9px] font-bold text-zinc-500 uppercase cursor-help border-b border-dashed border-zinc-700">Mức độ sáng tạo (Creative Drift)</label>
                  {/* TOOLTIP */}
                  <div className="absolute bottom-full left-0 mb-2 w-56 p-2 bg-zinc-800 text-[9px] text-zinc-300 rounded-lg border border-zinc-700 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
           ...
           <div className="relative h-4 flex items-center">
             <input 
                ...
                className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-orange-600 hover:accent-orange-500 z-20 relative"
             />
              <div className="absolute inset-0 flex justify-between px-1 pointer-events-none z-10">
                {[...Array(10)].map((_, i) => <div key={i} className={`w-0.5 h-1.5 rounded-full ${i+1 <= creativeDrift ? 'bg-orange-600/50' : 'bg-zinc-800'}`}></div>)}
             </div>
           </div>
         </div>
         ...
         <div>
           ...
           <div className="relative h-4 flex items-center">
             <input 
                ...
                className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-blue-600 hover:accent-blue-500 z-20 relative"
             />
             <div className="absolute inset-0 flex justify-between px-1 pointer-events-none z-10">
                {[...Array(10)].map((_, i) => <div key={i} className={`w-0.5 h-1.5 rounded-full ${i+1 <= batchSize ? 'bg-blue-600/50' : 'bg-zinc-800'}`}></div>)}
             </div>
           </div>
         </div>
      </div>
